...
const ServerSearch = ({
    ...
    const onClick = ({id, type}: {id: string, type: "member" | 'channel'}) => {
        setOpen(false)
        if (type == 'member') {
            return router.push(`/servers/${params?.serverId}/converstation/${id}`)
        }
    ...
    return (
        <>
            <button onClick={() => setOpen(true)} className="group px-2 py-2 rounded-md flex items-center gap-x-2 w-full hover:bg-zinc-700/10 dark:hover:bg-zinc-700/50 transition">
                ...
                <p className="font-semibold text-sm text-zinc-500 dark:text-zinc-400 group-hover:text-zinc-600 transition dark:group-hover:text-zinc-300">Search</p>
